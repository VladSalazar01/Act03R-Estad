...
Unifique los dos codigos el mio y el tuyo
                                                                                                  
                                                                                                  
```{r}
# Escalar los datos de expresión génica
# 'gene_expression_cols' es el dataframe de genes obtenido en la sección de preprocesamiento.
genes_scaled <- scale(genes)
                                                                                                  
# Aplicar el PCA
pca_result <- prcomp(genes_scaled, center = TRUE, scale. = TRUE)
                                                                                                  
# Obtener los eigenvalues (autovalores) y varianzas
eigenvalues_pca <- get_eigenvalue(pca_result)
varianzas <- pca_result$sdev^2
total_varianza <- sum(varianzas)
varianza_explicada <- varianzas / total_varianza
varianza_acumulada <- cumsum(varianza_explicada)
...
tabla_varianza %>%
gt() %>%
tab_header(title = md("**Varianza Explicada y Acumulada por Componente Principal**")) %>%
cols_label(
Componente = md("**Componente**"),
Varianza_Explicada = md("**% Varianza Explicada**"),
Varianza_Acumulada = md("**% Varianza Acumulada**")
) %>%
fmt_number(columns = c(Varianza_Explicada, Varianza_Acumulada), decimals = 2, suffix = "%")
                                                                                                  
# Determinar cuántos componentes explican al menos el 70% de la varianza
                                                                                                  n.pc <- min(which(varianza_acumulada > 0.7))
                                                                                                  
# Gráfico de Sedimentación (Scree Plot)
fviz_eig(pca_result, addlabels = TRUE, ylim = c(0, 40)) +
labs(title = "Gráfico de Sedimentación (Scree Plot)",
subtitle = "Varianza Explicada por cada Componente Principal",
...
fviz_contrib(pca_result, choice = "var", axes = 2, top = 10) +
labs(title = "Top 10 Genes Contribuyentes a la Dimensión 2") +
theme_minimal(base_size = 12) + theme(plot.title = element_text(hjust = 0.5))
...
# Cargas de los dos primeros componentes
cargas_pca <- as.data.frame(pca_result$rotation[, 1:2]) # Solo PC1 y PC2
cargas_pca
```
## Tablas Descriptivas de datos sin transformar


## Tablas Descriptivas de PCA, en terciles (ajustable)
...
generar_tabla_pc <- function(i) {
  ...
  if(var_tercil %in% names(datos_completos)) {
    datos_completos %>%
      ...
      modify_footnote(
        update = everything() ~ "Nota: Media (desviación estándar). p-value obtenido por ANOVA."
      )
  } else {
    return(NULL)
  }
